...
$sql_carrito = "SELECT productos.codigo, productos.nombrep, productos.precio FROM productos INNER JOIN carrito ON productos.codigo = carrito.productos_id";
$resultado_carrito = $conexion->query($sql_carrito);

if ($resultado_carrito->num_rows > 0) {
    while ($row = $resultado_carrito->fetch_assoc()) {
        $pdf->Cell(0, 10, 'Codigo: ' . $row['codigo'], 0, 1, 'C');
